...
def xsrf_secret_key():
    ...
    if not secret:
        ...
        secret = model.secret
        memcache.add(
            XSRF_MEMCACHE_ID, secret, namespace=OAUTH2CLIENT_NAMESPACE)
...
class AppAssertionCredentials(AssertionCredentials):
    ...
    def __init__(self, scope, **kwargs):
        ...
        # Assertion type is no longer used, but still in the parent class
        # signature.
...
class StorageByKeyName(Storage):
    ...
    def __init__(self, model, key_name, property_name, cache=None, user=None):
        ...
        if key_name is None:
            if user is None:
                raise ValueError(
                    'StorageByKeyName called with no key name or user.')
    ...
    def _is_ndb(self):
        ...
        raise TypeError(
            'Model class not an NDB or DB model: %s.' % (self._model,))
...
class OAuth2Decorator(object):
    ...
    def oauth_required(self, method):
        ...
        def check_oauth(request_handler, *args, **kwargs):
            ...
            # Don't use @login_decorator as this could be used in a POST
            # request.
            if not user:
